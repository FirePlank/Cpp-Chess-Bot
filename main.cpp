// DONT MIND ME JUST IMPORTING THIS HUGE ASS CHESS LIBARY
// MINGW WONT COMPILE WITH HEADER FILES FOR SOME REASON SO I HAD TO PUT EVERYTHING IN THE MAIN FILE LMAOO
// MY WRITTEN CODE IS AT THE VERY END
...
vector<int> find(string sample, char findIt)
{
...
int evaluation(thc::ChessRules &cr, bool maximazing_white) {
    ...
    int black_score = 0;
    int pieces = 0;
    string board = cr.ToDebugStr().c_str();
    board = board.substr (14,77); 
    board.erase(remove(board.begin(), board.end(), '\n'), board.end());

    // Piece square maps and piece values
    for (unsigned int i=0; i < 65; i++) {
        ...
        switch(piece) {
            case 'p' :
                ...
                if (FIRST_END) {
                    black_score += PAWN_SQUARE_EVAL[64 - 8 * (i >> 3) + i % 8 - 8];
                } else {
                    black_score += END_PAWN_SQUARE_EVAL[64 - 8 * (i >> 3) + i % 8 - 8];
                }
                break;
            case 'P':
                white_score += pawn_value;
                if (FIRST_END) {
                    white_score += PAWN_SQUARE_EVAL[i];
                } else {
                    white_score += END_PAWN_SQUARE_EVAL[i];
                }
                break;
            case 'r':
                black_score += rook_value;
                pieces += 1;
                black_score += ROOK_SQUARE_EVAL[64 - 8 * (i >> 3) + i % 8 - 8];
                break;
            case 'R':
                white_score += rook_value;
                pieces += 1;
                white_score += ROOK_SQUARE_EVAL[i];
                break;
            case 'n':
                black_score += knight_value;
                pieces += 1;
                black_score += KNIGHT_SQUARE_EVAL[64 - 8 * (i >> 3) + i % 8 - 8];
                break;
            case 'N':
                white_score += knight_value;
                pieces += 1;
                white_score += KNIGHT_SQUARE_EVAL[i];
                break;
            case 'b':
                black_score += bishop_value;
                pieces += 1;
                black_score += BISHOP_SQUARE_EVAL[64 - 8 * (i >> 3) + i % 8 - 8];
                break;
            case 'B':
                white_score += bishop_value;
                pieces += 1;
                white_score += BISHOP_SQUARE_EVAL[i];
                break;
            case 'q':
                black_score += queen_value;
                pieces += 2;
                black_score += QUEEN_SQUARE_EVAL[64 - 8 * (i >> 3) + i % 8 - 8];
                break;
            case 'Q':
                white_score += queen_value;
                pieces += 2;
                white_score += QUEEN_SQUARE_EVAL[i];
                break;
            case 'k':
                if (FIRST_END) {
                    black_score += KING_SQUARE_EVAL[64 - 8 * (i >> 3) + i % 8 - 8];
                } else {
                    black_score += END_KING_SQUARE_EVAL[64 - 8 * (i >> 3) + i % 8 - 8];
                }
                break;
            case 'K':
                if (FIRST_END) {
                    white_score += KING_SQUARE_EVAL[i];
                } else {
                    white_score += END_KING_SQUARE_EVAL[i];
                }
                break;
        }
    }
    if ( FIRST_END && pieces < 5) {
        FIRST_END = false;  
        knight_value = 340;
        pawn_value = 200;
    } 
...
int negamax(thc::ChessRules &cr, std::deque<std::__cxx11::basic_string<char>> valid_moves, int size, int depth, int alpha, int beta, bool maximizing_white, int DEPTH, int origin_max) {
    int max_score = -999999999;

    if (depth == 0) {
        return evaluation(cr, maximizing_white);
    }
    ...
    for (unsigned int i = 0; i < size; i++) {
        ...
        vector<bool> mate;
        vector<bool> stalemate;
        cr2.GenLegalMoveList(  next_moves, check, mate, stalemate );
...
string find_best_move(thc::ChessRules &cr) {
    ...
    std::deque<string> all_moves;
    for( unsigned int i=0; i<len; i++ ) {
        mv.TerseIn( &cr, next_moves[i].TerseOut().c_str());
        if (mate[i]) {
            return next_moves[i].TerseOut().c_str();
        } else if (check[i] || mv.capture != ' ') {
            all_moves.push_front(next_moves[i].TerseOut().c_str());
        } else {
            all_moves.push_back(next_moves[i].TerseOut().c_str());
...
int main() {
    ...
    while(true) {
        ...
        if (input == "quit") {
            ...
            cout << "id name FireFullBot" << endl;
            cout << "id author FireFullBot's developer" << endl;
            cout << "uciok" << endl;
        } else if (input == "isready") {
            cout << "readyok" << endl;
        } else if (input == "ucinewgame") {
            continue;
        } else if (input.rfind("position startpos moves", 0) == 0) {
            cr = cr2;
